...
**`GUI.bat` をダブルクリック** すればブラウザが開いて、そこから全部できる。
コマンドで使うなら:
...
| .bat | 何をするか |
|---|---|
| **`GUI.bat`** | 残骸を片付け → 依存を確認 → ブラウザで GUI を開く |
| `measure.bat` | 残骸を片付け → 測定リグを起動（Minecraft から `localhost:25566` に繋ぐ） |
| `stop.bat` | mcnb が残したプロセスを止める |

**起動前に必ず前回の残骸を片付ける。** 止めるのは
「指定したポートを実際に LISTEN しているプロセス」と
「このリポジトリのサーバ jar を動かしている java」だけなので、
**遊んでいる Minecraft クライアントには触らない**。

バッチは **ASCII のみ**にしてある。日本語の表示は Python 側が担当する。
cmd の文字コードは環境によって cp932 だったり UTF-8 だったりして、
バッチに日本語を書くと化けるため。
...
音符ブロックは**通り道の左右の壁**に並べる（真上には積まない）。
壁は 5 段 × 左右 ±45 ブロック。真上に積むほうが定位を中央に保てるが、
左右のほうが**見える**し、参考にしている作品もそうしている。
横方向は距離を連続的に取れるので、**狙った音量に正確に置ける**
（音量誤差 0.09 → 0.04 に改善した）。

- **プレイヤーは見えない台（armor_stand）に乗って動く。** プレイヤーを直接 `/tp` すると
  位置が毎 tick スナップして **20fps のカクつき**になるが、エンティティの動きは
  クライアントが補間するので滑らかに見える。向きは `/rotate`（テレポートを伴わない）で固定
- **移動速度は曲の密度から自動で決まる。** 速度そのものは音にほぼ影響しない（実測）が、
  1つの平面に何 tick 詰め込むかが変わり、詰め込みすぎると音が置けなくなる
